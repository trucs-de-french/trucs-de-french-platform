...
export function SwappableTileRow({
  ...
  function tileClass(i: number) {
    ...
    if (state === "correct") return "border-green-500 bg-green-50 dark:bg-green-950/30";
    if (state === "incorrect") return "border-red-500 bg-red-50 dark:bg-red-950/30";
    if (selected === i) return SELECTED_OPTION_CLASS;
    if (dragOverIndex === i) return "border-blue-400 bg-blue-50 dark:border-blue-500 dark:bg-blue-950/30";
    ...
    <div className="flex flex-wrap gap-2">
      {items.map((text, i) => (
        <button
          ...
          onClick={() => clickTile(i)}
          disabled={locked}
          className={`cursor-grab select-none rounded-md border px-3 py-1.5 text-sm active:cursor-grabbing disabled:cursor-not-allowed disabled:opacity-70 ${tileClass(i)}`}
        >
          {text}
        </button>
      ))}
